...
// selection length calibration (can be tweaked if a device shows a tiny fixed offset)
const FUDGE_SAMPLES = 0 // e.g., try 128 or 256 on some devices
...
export function extractWithSnapshot(ch, startFrac, endFrac, snap) {
  ...
  let b = Math.max(0, Math.min(1, Number(endFrac)   || 0))
  if (b < a) [a, b] = [b, a]
  ...
  const r = Math.min(1, usable / Math.max(1, windowFrames))        // fraction of the window that actually has audio
  const leftPadFrac = 1 - r                                        // left blank area fraction
  const scale = r > 0 ? (1 / r) : 1
  // Map selection across whole window -> selection within the usable right-aligned region
  const a2 = Math.max(0, Math.min(1, (a - leftPadFrac) * scale))
  const b2 = Math.max(0, Math.min(1, (b - leftPadFrac) * scale))

  const startFrames = Math.max(0, Math.min(usable, Math.round(a2 * usable)))
  const endFrames   = Math.max(0, Math.min(usable, Math.round(b2 * usable)))
