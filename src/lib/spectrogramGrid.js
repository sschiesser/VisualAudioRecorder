...
export function setupGrid(containerSelector='#spectrograms',chCount=2){
  ...
  holders.forEach((holder,i)=>{
    ...
    if(!recBtn){ recBtn = document.createElement('div'); recBtn.className='recBtn'; recBtn.textContent='● REC'
      Object.assign(recBtn.style,{position:'absolute',left:'8px',top:'28px',padding:'2px 8px',borderRadius:'10px',border:'1px solid #2b425a',cursor:'pointer',userSelect:'none',background:'#1b2635'})
    ...
    if(!overlay){overlay=document.createElement('div');overlay.className='overlay';overlay.style.display='none'
      ...
      const loopBtn=document.createElement('div');loopBtn.className='btn';loopBtn.textContent='⟲ Loop'; loopBtn.onclick=()=>{ loopFlags[i]=!loopFlags[i]; loopBtn.style.borderColor = loopFlags[i] ? '#36d67e' : '#2b425a' }
      const hint=document.createElement('div');hint.className='hint';hint.textContent='Wheel=Zoom, Drag=Select, Alt+Drag=Pan'
      overlay.appendChild(playBtn);overlay.appendChild(saveBtn);overlay.appendChild(loopBtn);overlay.appendChild(hint);holder.appendChild(overlay)}
    ...
    holder.style.display=(i<chCount)?'block':'none';slider.style.display=(i<chCount)?'block':'none';gLabel.style.display=(i<chCount)?'block':'none';recBtn.style.display=(i<chCount)?'inline-block':'none'
  ...
  freqData=new Array(chCountReal);timeData=new Array(chCountReal)
  lastW=new Array(chCountReal).fill(0);lastH=new Array(chCountReal).fill(0)
  pxPerFrame=canvases.map(c=>((c?.width||300)/(windowSeconds*sampleRate)))
  frameAcc=new Array(chCountReal).fill(0)
  zoom=new Array(chCountReal).fill(1);viewStart=new Array(chCountReal).fill(0);selection=new Array(chCountReal).fill(null);frozen=new Array(chCountReal).fill(null)
  recArmed=new Array(chCountReal).fill(true); loopFlags=new Array(chCountReal).fill(false)
  // wire REC buttons
...
function drawTimeAxis(ctx, w, h, hWave, hSpec){
  ...
  for(let i=0;i<chCount;i++){
    ...
    const draw=()=>{
      ...
      const hAvail = Math.max(1, h - TICK_H)
      const hWave=Math.max(30,Math.round(hAvail*WAVE_RATIO)),hSpec=Math.max(1,hAvail-hWave)
      if(!isPlaying())return
      ...
      if(steps>0){
        ...
        for(let s=steps;s>0;s--){const x=w-s
          ...
          // spectrogram
          for(let yy=0;yy<hSpec;yy++){const bin=Math.floor((yy/hSpec)*freqData[i].length);const v=freqData[i][bin];ctx.fillStyle=hueHeat(v);ctx.fillRect(x,h-TICK_H-1-yy,1,1)}
          // time axis column
          ...
          // axis background
          ctx.fillStyle = '#0b131e'; ctx.fillRect(x, h - TICK_H, 1, TICK_H)
          if (isMajor) {
            // vertical grid line
            ctx.fillStyle = 'rgba(124,197,255,0.25)'; ctx.fillRect(x, 0, 1, hAvail)
            // tick + label
            ctx.fillStyle = '#7cc5ff'; ctx.fillRect(x, h - TICK_H, 1, 6)
...
export function enterInspectMode(){inspect=true;canvases.forEach((canvas,i)=>{if(!canvas)return;const off=document.createElement('canvas');off.width=canvas.width;off.height=canvas.height;off.getContext('2d').drawImage(canvas,0,0);frozen[i]=off;const holder=canvas.parentElement;if(holder.__overlay)holder.__overlay.style.display=selection[i]?'flex':'none';if(holder.__renderPaused)holder.__renderPaused()})}
export function exitInspectMode(){inspect=false;canvases.forEach((canvas)=>{const holder=canvas?.parentElement;if(holder?.__overlay)holder.__overlay.style.display='none';if(holder?.__selDiv)holder.__selDiv.style.display='none'})}
export function bindSelectionHandlers(h){handlers={...handlers,...h}}
export function setPauseSnapshotMeta(snap){ snapMeta = snap }
export function bindRecordToggles(setter){ setRecCallback = setter }
